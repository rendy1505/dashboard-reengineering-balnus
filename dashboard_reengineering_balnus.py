...
#
# site_inventory/hw_inventory (Hardware Inventory menu, ~75MB combined)
# were originally kept out of this dict and lazy-loaded on demand instead,
# specifically to avoid pushing the combined payload to ~145MB. Folded
# back in per explicit request to auto-load on every login instead of
# requiring a manual "Load Hardware Inventory Data" click — if OOM crashes
# or "connection reset by peer" health-check failures show up in the logs,
# this pair is the first thing to pull back out into a lazy-load again.
